...
extern int ZEXPORT zipOpenNewFileInZip4_64(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw, int windowBits, int memLevel,
    int strategy, const char *password, uint32_t crc_for_crypting, uint16_t version_madeby, uint16_t flag_base, int zip64)
{
    ...
    if (filename == NULL)
        filename = L"-";
    if (comment != NULL)
        size_comment = (uint16_t)wcslen(comment);

    // try to pack utf8 bytes as char sequence
    // how many bytes do we need to do this?
    // 2 bytes per symbol is enough
    size_filename = (uint16_t)wcslen(filename);
    size_filename = size_filename * 2;
    // convert filename
    char filenameaschar[size_filename];
    memset(filenameaschar,'\0',size_filename*sizeof(char));
    uint16_t tmpindex=0;
    ...
    for (int i=0; i<size_filename; i++) {
        if (i % 2 == 0)
        {
            ...
            if (lb == 0x5C)
                lb=0x2F;
            filenameaschar[tmpindex]=(char)lb;
            filenameaschar[tmpindex+1]=(char)hb;
            tmpindex+=2;
        }
        else {
            // second byte of utf8
    ...
    for (i = 0; i < size_filename; i++)
        zi->ci.central_header[SIZECENTRALHEADER+i] = filenameaschar[i];
    ...
    {
        if (ZWRITE64(zi->z_filefunc, zi->filestream, filenameaschar, size_filename) != size_filename)
            err = ZIP_ERRNO;
    }
...
extern int ZEXPORT zipOpenNewFileInZip4(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw, int windowBits,
...
extern int ZEXPORT zipOpenNewFileInZip3(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw, int windowBits,
...
extern int ZEXPORT zipOpenNewFileInZip3_64(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw, int windowBits,
...
extern int ZEXPORT zipOpenNewFileInZip2(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
...
extern int ZEXPORT zipOpenNewFileInZip2_64(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int raw, int zip64)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
        extrafield_global, size_extrafield_global, comment, method, level, raw, -MAX_WBITS, DEF_MEM_LEVEL,
...
extern int ZEXPORT zipOpenNewFileInZip64(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level, int zip64)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
...
extern int ZEXPORT zipOpenNewFileInZip(zipFile file, const wchar_t *filename, const zip_fileinfo *zipfi,
    const void *extrafield_local, uint16_t size_extrafield_local, const void *extrafield_global,
    uint16_t size_extrafield_global, const wchar_t *comment, uint16_t method, int level)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
        extrafield_global, size_extrafield_global, comment, method, level, 0, -MAX_WBITS, DEF_MEM_LEVEL,
        Z_DEFAULT_STRATEGY, NULL, 0, VERSIONMADEBY, 0, 0);
